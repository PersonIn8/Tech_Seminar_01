...
## 📚기술 세미나 진행방식
1️⃣ 발표 자료 제작
- 캔바(Canva) 활용하여 PPT 제작
- 발표 내용을 시각적으로 정리하여 이해하기 쉽게 구성

2️⃣ 진행 기간
- 2025년 10월 20일 ~ 2026년 2월 17일
- 일정에 맞춰 단계별 준비 및 연습 진행

3️⃣ 학습 및 발표 준비
- 개인 공부를 통해 학습한 내용(책 참고 및 개념 정리) 발표
- 이론적인 내용을 중심으로 설명하여 팀원들의 이해를 돕는 방식
- 코드 구현 없이 개념 및 보안 이슈 중심으로 발표 진행

4️⃣ 발표 및 피드백
- 주말마다 슬랙을 통해 발표 영상 촬영 및 공유
- 팀원들의 피드백을 받아 발표 내용을 개선
- 지속적인 연습을 통해 발표 역량 향상
...
### ⁉️**쿠키의 속도는 왜 상대적으로 빠른 것일까? / 세션의 속도 저하는 왜 일어날까?**
- **서버에 저장**되어 있기 때문에 **사용자의 정보를 조회하고 식별**해야하기 때문에 쿠키나 jwt 보다 상대적으로 **속도가 저하**될 수 밖에 없음

<br>


### ⁉️**쿠키와 세션을 비교하는 사진에서 왜 2개의 요청이 존재할까?**
...
### ⁉️**왜 JWT와 Spring Security Framework가 같이 쓰이나요?**
...
### ⁉️**세션의 저장소가 서버라 해서 왜 쿠키 크기에 제한이 없어진다고 이야기 할 수 있나요?**
...
1️⃣ 아쉬운 점 😕
- ❌ 구현체가 없어서 아쉬움
- 이론 중심으로 발표가 진행되었으나, 실질적인 구현 사례 부족.
- 코드 예제나 실제 동작 방식에 대한 시연이 포함되면 더 좋았을 것.

2️⃣ 좋은 점 ✅
- 최종 결론 정리가 명확하게 잘 되어 있음.
- 공식 문서를 참고한 점이 좋았음.
- PPT가 깔끔하고, 사용하는 문구가 정제되어 있어 고민을 많이 한 흔적이 보여서 좋았음.
- 상황극을 활용하여 Stateless 상태를 쉽게 설명한 것이 인상적이었음.
- 비유를 활용한 설명이 효과적이었음 (예: Access & Refresh Token → 여권 & 비자).

3️⃣ 멘토님(실무자) 피드백 (추가 공부할 내용) 🧐
- 분산 서버 환경에서의 보안 관련 내용 학습 필요
- 실무에서는 토큰 방식보다 세션 방식을 더 많이 사용
    - JWT를 직접 사용하는 것보다, 세션에 토큰을 저장하고 세션 키만 전달하는 방식을 주로 사용.
    - 실무에서 주로 사용되는 인증 방식 🏢
      - 세션 기반 인증이 일반적
      - 세션에 토큰을 저장하고, 클라이언트에게는 세션 키만 전달하는 방식
      - 1️⃣ 사용자가 로그인하면 서버에서 JWT(또는 인증 정보를 담은 토큰)를 생성하고 세션에 저장.
      - 2️⃣ 클라이언트에게는 세션 키(Session ID 또는 Token ID)만 반환.
      - 3️⃣ 이후 클라이언트는 요청마다 Cookie 또는 Authorization 헤더에 세션 키를 포함하여 요청.
      - 4️⃣ 서버는 세션 키를 통해 저장된 토큰을 조회 및 검증 후 요청을 처리. <br>
->➡️ 보안성과 무효화의 용이성 때문.
- 🔐 JWE, JWS 방식 추가 학습 필요 <br>
  - JWS (JSON Web Signature) → ✅ 서명된 JWT <br>
